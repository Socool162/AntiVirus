/*
compile:        gcc -o antiVirus.exe antiVirus.c
Run:                antiVirus.exe 
Path:       C:\test      C:\cleaned_files
*/
#include <stdio.h>
#include <stdlib.h>     // For malloc, free
#include <string.h>     // For strncmp, strcpy_s, ...
...
/*
======================== Flow code ========================
Input: (`Scanned Path`) và (`Cleaned Path`)
Sau khi khởi tạo và mở file log, nó gọi hàm `scanDirectory`

Hàm `scanDirectory`:
    + duyệt đệ quy thư mục được chỉ định
    + lọc ra các file `.exe`, `.dll`, `.sys` 
    + Rồi chuyển từng file cho `processFile` xử lý. 

Hàm `processFile`:
    + đọc toàn bộ file vào buffer, phân tích cấu trúc PE,
    + thực hiện logic phát hiện: tìm một section tên là `.infect` 
    + kiểm tra xem Entry Point của file có đang trỏ vào section đó không

Nếu phát hiện lây nhiễm, nó gọi hàm `disinfect` để "vá" lại file trong bộ đệm 
bằng cách đọc OEP gốc được virus cất giữ (tại offset `0x9F` so với đầu stub) 
và khôi phục OEP này về PE Header. 

Hàm `disinfect` cũng giảm `NumberOfSections` và xóa header của section `.infect`. 
Cuối cùng, `processFile` ghi lại bộ đệm đã được vá (fileSize - size of section .infect) 
ra một file mới trong thư mục `Cleaned Path`.
*/
...
/**
 *  struct to hold the ScannerState 
 */
typedef struct {
    int totalScanned;
    int totalInfected;
...
int disinfect(ScannerState* state, BYTE* pFileBuffer, PIMAGE_NT_HEADERS ntHeaders, PIMAGE_SECTION_HEADER infectedSection) {
    ...
    //    We use the 0x9F constant found from the .lst file
...
void processFile(ScannerState* state, const char* filePath) {
    ...
    if (infectedSection) {
        ...
        if (disinfect(state, pFileBuffer, ntHeaders, infectedSection)) {
            ...
            // Write the modified buffer to the new file, check exists
            FILE* outFile = NULL;
            if (fopen_s(&outFile, cleanPath, "rb") == 0) {
                logMessage(state, "  [ERROR] File already exists: %s\n\n", cleanPath);
                fclose(outFile);
                state->totalCleaned--; // Revert counter
                return; // Skip cleaning this file
            } else {
...
void scanDirectory(ScannerState* state, char* dirPath) {
    ...
    // Create the search path (e.g., C:\test\*)
...
int main() {
    ...
    printf("===================\n");
    printf("  Scan Path: ");
    if (fgets(scanDir, MAX_PATH, stdin) == NULL) {
        fprintf(stderr, "[ERROR] Fail to read Scanned Path\n");
        return 1;
    }
    scanDir[strcspn(scanDir, "\n\r")] = 0;  // Remove newline character

    printf("  Cleaned Path: ");
    if (fgets(outDir, MAX_PATH, stdin) == NULL) {
        fprintf(stderr, "[ERROR] Fail to read Cleaned Path\n");
        return 1;
    }
    outDir[strcspn(outDir, "\n\r")] = 0;
